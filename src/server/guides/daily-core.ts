// FILE: src/server/guides/daily-core.ts
import "server-only";
...
function isMarsKetu(t?: CoreTransitSignal | null): boolean {
  ...
  return planet === "mars" && /natal\s+ketu/.test(target);
}
...
function buildEmotionalWeather(core: CoreSignals): any {
  ...
  // NOTE: EmotionalWeather in your types expects string[] for workWithIt / avoid
  if (tone === "intense") {
    return {
      quality: "intense",
      summary:
      ...
      workWithIt: [
        "Pause before big conversations and give yourself space to breathe.",
        "Channel intensity into one clear, constructive task.",
      ],
      avoid: [
        "All-or-nothing decisions or revenge thinking.",
        "Doom-scrolling or letting yourself spiral in mental loops.",
      ],
    };
  }
  ...
  if (tone === "sensitive") {
    return {
      ...
      workWithIt: [
        "Slow the pace a little and choose gentle company and media.",
        "Give yourself permission to feel without judging yourself.",
      ],
      avoid: [
        "Over-exposure to stressful news or harsh environments.",
        "Forcing big life decisions when you’re not feeling clear.",
      ],
    };
  }
  ...
  if (tone === "low") {
    return {
      ...
      workWithIt: [
        "Lower the bar and focus on one or two small meaningful tasks.",
        "Build in extra rest and simple grounding routines.",
      ],
      avoid: [
        "Comparing your pace to others or labelling yourself as ‘lazy’.",
        "Overloading the schedule just to avoid uncomfortable feelings.",
      ],
    };
  }
  ...
  return {
    ...
    workWithIt: [
      "Schedule important tasks and key conversations at a steady pace.",
      "Clear one or two pending items without rushing yourself.",
    ],
    avoid: [
      "Over-scheduling yourself so there is no breathing space.",
      "Ignoring subtle emotional signals or over-promising.",
    ],
  };
}
...
function buildFoodGuide(
  ...
  const tone =
    (emotionalWeather as any)?.tone ??
    (emotionalWeather as any)?.quality ??
    "steady";
  ...
  if (!weekdayLord) {
    try {
      const iso =
        (core.birth?.dateISO || new Date().toISOString().slice(0, 10)) as string;
