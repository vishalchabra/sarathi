// IMPORTANT: no `import "server-only"` in this file.
// This file is safe to import from Node scripts/tests.
...
export async function vimshottariMDTable(birth: Birth): Promise<MDT[]> {
  try {
    // ✅ Prefer caller-provided sidereal Moon longitude (from life-engine planets)
    const moonFromCaller =
      typeof birth.moonSiderealLongitude === "number" &&
      Number.isFinite(birth.moonSiderealLongitude)
        ? birth.moonSiderealLongitude
        : null;
