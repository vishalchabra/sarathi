...
// Temporary stub – ascAndHouseLords was removed from astro/core.
// This keeps BCP compiling until we wire a proper implementation.
const ascAndHouseLords: any = (..._args: any[]) => ({} as any);
// Temporary stub for PLANET_NAMES – real mapping lives in astro/nakshatra.
// This keeps BCP compiling without pulling in extra dependencies.
...
export function buildBhriguPulses(birth: BirthInput, years = 7) {
  // Current age (floor) in the person’s timezone
  ...
  // Asc & house rulers so we can show meaningful "why" lines
  const houseBasics = ascAndHouseLords(birth);
  const ascSign = houseBasics?.asc?.ascSign ?? 0;

  const items: BCPItem[] = [];
  for (let i = 0; i < years; i++) {
    const A = age + i; // age window
    const start = born.plus({ years: A });
    const end = start.plus({ years: 1 });

    // Classic “one age → one house” pulse
    ...
    items.push({
      ...
      why: [
        ...
        `Watch its lord: ${lordName}. Planets in ${house}H or aspecting it will time events.`,
      ],
    });
  }
