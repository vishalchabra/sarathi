...
function enrichNak(pos: PlanetPos): PlanetPos {
  if (!pos.nakName) {
    ...
    return {
      ...pos,
      nakName: nk.name,
      // we no longer read nk.pada here – getNakshatra doesn’t expose it
    };
  }
...
export async function POST(req: Request) {
  try {
    ...
    const pack = buildVenusPack(positions);
    return NextResponse.json(pack);
  } catch (e: any) {
    return NextResponse.json({ error: e?.message || "Venus API error" }, { status: 500 });
  }
}
