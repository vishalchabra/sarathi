...
export function VenusCard({ data }: { data: any }) {
  const { natal, features, scores, windows, explain, remedies } = data || {};

  return (
    <div className="p-4 rounded-xl shadow bg-white space-y-3">
      <h2 className="text-xl font-bold">
        ♀ Venus — {natal?.sign} • House {natal?.house}
        {natal?.nak ? ` • ${natal.nak.name} (p${natal.nak.pada})` : ""}
      </h2>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Score label="Love" value={scores?.love ?? 0} />
        <Score label="Beauty" value={scores?.beauty ?? 0} />
        <Score label="Finance" value={scores?.finance ?? 0} />
        <Score label="Attachment" value={scores?.attachment ?? 0} />
      </div>
      ...
      <div className="text-sm text-gray-700 space-y-1">
        ...
        {features?.aspects_on_venus?.length > 0 && (
          <p><b>Aspected by:</b> {features.aspects_on_venus.map((a: any) => `${a.from} (${a.kind})`).join(", ")}</p>
        )}
      </div>
