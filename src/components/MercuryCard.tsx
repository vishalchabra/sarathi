...
// keep prop loose for now; we’ll add strict types later
export function MercuryCard({ data }: { data: any }) {
  const { natal, features, scores, windows, explain, remedies } = data || {};

  return (
    <div className="p-4 rounded-xl shadow bg-white space-y-3">
      <h2 className="text-xl font-bold">
        ☿ Mercury — {natal?.sign} • House {natal?.house}
        {natal?.nak ? ` • ${natal.nak.name} (p${natal.nak.pada})` : ""}
      </h2>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Score label="Intellect" value={scores?.intellect ?? 0} />
        <Score label="Business" value={scores?.business ?? 0} />
        <Score label="Comms" value={scores?.communication ?? 0} />
        <Score label="Nerves" value={scores?.nerves ?? 0} />
      </div>
      ...
      <div className="text-sm text-gray-700 space-y-1">
        ...
        {features?.aspects_on_mercury?.length > 0 && (
          <p><b>Aspected by:</b> {features.aspects_on_mercury.map((a: any) => `${a.from} (${a.kind})`).join(", ")}</p>
        )}
      </div>
