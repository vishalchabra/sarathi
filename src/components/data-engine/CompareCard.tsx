...
export default function CompareCard({ compare, isPro = false }: Props) {
  if (!isPro) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-base font-semibold text-slate-950">Compare Mode</h2>
  ...
  if (!compare) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-base font-semibold text-slate-950">Compare Mode</h2>
        <div className="mt-3 text-sm text-slate-500">No compare data available.</div>
      </div>
    );
  }
    ...
    <tbody>
      {natalRows.length ? (
        natalRows.map((row: any, idx: number) => (
          <tr key={`${row?.planet ?? "p"}-${idx}`} className="border-b border-slate-100">
