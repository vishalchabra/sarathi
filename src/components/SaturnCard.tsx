...
export function SaturnCard({ data }: { data: any }) {
  const { natal, features, scores, windows, explain, remedies } = data || {};

  return (
    <div className="p-4 rounded-xl shadow bg-white space-y-3">
      <h2 className="text-xl font-bold">
        ♄ Saturn — {natal?.sign} • House {natal?.house}
        {natal?.nak ? ` • ${natal.nak.name} (p${natal.nak.pada})` : ""}
      </h2>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Score label="Discipline" value={scores?.discipline ?? 0} />
        <Score label="Patience" value={scores?.patience ?? 0} />
        <Score label="Karma" value={scores?.karma ?? 0} />
        <Score label="Delay" value={scores?.delay ?? 0} />
      </div>
      ...
      <div className="text-sm text-gray-700 space-y-1">
        ...
        {features?.aspects_on_saturn?.length > 0 && (
          <p><b>Aspected by:</b> {features.aspects_on_saturn.map((a: any) => `${a.from} (${a.kind})`).join(", ")}</p>
        )}
      </div>
