...
export function KetuCard({ data }: { data: any }) {
  const { natal, features, scores, windows, explain, remedies } = data || {};

  return (
    <div className="p-4 rounded-xl shadow bg-white space-y-3">
      <h2 className="text-xl font-bold">
        ☋ Ketu — {natal?.sign} • House {natal?.house}
        {natal?.nak ? ` • ${natal.nak.name} (p${natal.nak.pada})` : ""}
      </h2>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Score label="Detachment" value={scores?.detachment ?? 0} />
        <Score label="Spirituality" value={scores?.spirituality ?? 0} />
        <Score label="Insight" value={scores?.insight ?? 0} />
        <Score label="Loss" value={scores?.loss ?? 0} />
      </div>
      ...
      <div className="text-sm text-gray-700 space-y-1">
        ...
        {features?.aspects_on_ketu?.length > 0 && (
          <p><b>Aspected by:</b> {features.aspects_on_ketu.map((a: any) => `${a.from} (${a.kind})`).join(", ")}</p>
        )}
      </div>
